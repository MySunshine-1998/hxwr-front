...
import { status, value, RequestEnum, ResultEnum, ContentTypeEnum, errorMessages } from './enum'
  ...
  transformResponseHook: (res, options) => {
    ...
    switch (code) {
      case ResultEnum.TIMEOUT:
        timeoutMsg = errorMessages.timeoutMessage
        break
      default:
        if (message) {
          timeoutMsg = message
    ...
    throw new Error(timeoutMsg || errorMessages.apiRequestFailed)
  },
  ...
  responseInterceptorsCatch: (axiosInstance, error) => {
    ...
    try {
      if (code === 'ECONNABORTED' && message.indexOf('timeout') !== -1) {
        errMessage = errorMessages.apiTimeoutMessage
      }
      if (err?.includes('Network Error')) {
        errMessage = errorMessages.networkExceptionMsg
      }
